...
- **Store the forecast fields now, score them later.** The ingest keeps six features the score does not read: `swell_peak_period_s`, `combined_height_m`/`combined_period_s`, and the `swell2_*` trio. The reason is asymmetric recoverability — ERA5 can always say what the ocean *did*, and nothing can say what we *believed it would do* on a day the worker did not store it. Adding one needs no migration (the payload is JSONB, absent keys re-hydrate as `None`) but `assemble.forecasts_from_grid` enumerates fields by hand, so a new one must be carried through to `HourForecast` or the harness will never see it. Using any of them in the score is a Stage 3 change and needs a backtest number.
- **Snapshots record beliefs, not polls.** `persist_hours` appends a `forecast_snapshots` row only when the payload differs from the last one for that (cell, hour) — two cycles 23 minutes apart once wrote 1176 rows and 1176 identical payloads. This is lossless for an as-of query, which takes the newest row with `fetched_at <= T`: the retained row's stamp becomes when a belief *started* rather than when it was last restated. `forecast_current` is upserted every cycle regardless, because its `fetched_at` means "we checked" and is what freshness is read from. Do not make either one follow the other.
- **`forecast_current` is a cache of hours still ahead; `forecast_snapshots` is the record.** `load_current_hours` is bounded and `prune_current` deletes on `valid_at`, never on `fetched_at` — so a spell where Open-Meteo is unreachable degrades by going out of date, and never by having future hours deleted underneath it.
- **One handle for one person, from `store.default_handle`.** An observation is unique on `(user_id, spot_id, started_at)`, so two handles do not collide, they duplicate — the page and `gogo log` writing as different people would turn one session into two labels, which is exactly what 004 exists to prevent and cannot detect. Never reintroduce a second default.
- **`gogo health` is how the box is asked whether it is working.** Both ways this system fails are silent: a stopped worker leaves a page that still renders, and a spot that drops out of the ranking leaves one fewer row in a list nobody counts. Exit 1 means one of those happened. It is the worker container's healthcheck; Docker will mark it unhealthy and deliberately will not restart it, because the loop is built to survive a bad hour rather than exit on one. It also carries the heartbeat: with `GOGO_HEARTBEAT_URL` set it pings a dead-man's switch **only when healthy**, so the alert is the ping stopping — the one signal that still arrives when the container, the disk or the whole box is gone.
